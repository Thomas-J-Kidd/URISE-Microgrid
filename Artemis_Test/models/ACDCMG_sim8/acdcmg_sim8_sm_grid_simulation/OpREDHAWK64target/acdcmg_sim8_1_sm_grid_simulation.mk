...
MODEL           = acdcmg_sim8_1_sm_grid_simulation
MODULES         = rt_matrx.c rt_printf.c rt_logging.c acdcmg_sim8_1_sm_grid_simulation_data.c acdcmg_sim8_1_sm_grid_simulation_offsets.c rtGetInf.c rtGetNaN.c rt_look.c rt_look1d.c rt_nonfinite.c OP_SEND.c RECV_Param.c fts5stub.c sfun_discreteVariableDelay.c ssn_sfun_v1.c
MAKEFILE        = acdcmg_sim8_1_sm_grid_simulation.mk
...
MODELLIB                  = 
MODELREF_LINK_LIBS        = 
MODELREF_LINK_RSPFILE     = acdcmg_sim8_1_sm_grid_simulation_ref.rsp
...
SFS_TMP84 = $(SFS_TMP79:simulation_events.c=)
SFS_TMP88 = $(SFS_TMP84:sfun_sync_vme200_2.c=)
SFS_TMP93 = $(SFS_TMP88:sfun_subsystem_trigger.c=)
...
SFS_TMP129 = $(SFS_TMP112:error_status.c=)
...
SFS_TMP240 = $(SFS_TMP238:nrt.c=)
SFS_TMP242 = $(SFS_TMP240:pubhlaclassattr.c=)
...
# Note: insted of using MODULE_SRC and S_FUNCTION_SRC, just use one variable to hold
# all the source files. 
ALL_SRCS = $(SFS_TMP283:optaketime.c=)
...
ifeq ($(MODELREF_TARGET_TYPE), NONE)
  # Top model for RTW  
  SRCS1               += $(MODEL).c $(WRAPPER_FILE) rt_sim.c $(RTWLOG)  $(SOLVER) $(EXT_SRC) $(ALL_SRCS) $(SHARED_SRC)
  SRCS1               += $(INTERNAL_ADD_SOURCE_FILE)  $(USER_SRCS) 
...
# Less recent MATLAB versions are in rtlab_rtmodel_pre_r2019.tmf
ifeq ($(OP_MATLABR2019A),1)
MATLABVERSION_CFLAGS = -DOP_MATLABR2019A
endif
ifeq ($(OP_MATLABR2019B),1)
MATLABVERSION_CFLAGS = -DOP_MATLABR2019B
endif
ifeq ($(OP_MATLABR2020A),1)
MATLABVERSION_CFLAGS = -DOP_MATLABR2020A
endif
ifeq ($(OP_MATLABR2020B),1)
MATLABVERSION_CFLAGS = -DOP_MATLABR2020B
endif
...
CPPFLAGS   = $(CC_OPTS) $(CPP_REQ_DEFINES) $(TARGET_CPPFLAGS) $(INCLUDES)
...
LDFLAGS = $(SHARED_LIB_FLAG) $(LD_DEBUG_OPTS) $(LIBPATH) $(RTLAB_LDFLAGS)
...
ifeq "$(UNAME)" "Linux"	##### Linux (RedHawk)
	...
	ifeq ($(CC), gcc)
		ifeq ($(MODELREF_TARGET_TYPE), NONE)
			...
			include $(RELATIVE_PATH_TO_ANCHOR)/OpNTtarget/mingw.rules
		endif
	...
	ifeq ($(USE_EXPAND_RTWLIB),1)
		...
		ifeq ($(CC), gcc)
			MODULES_rtwlib = rt_backsubcc_dbl.o \
					rt_backsubcc_sgl.o \
					rt_backsubrc_dbl.o \
					rt_backsubrc_sgl.o \
					rt_backsubrr_dbl.o \
	...
	endif  # ($(USE_EXPAND_RTWLIB),1)

#------------------------------------------------------------------------------
# Rule for RTWLIB (Windows only)
#------------------------------------------------------------------------------
$(RTWLIB) :
	@echo ### Creating $@
...
ifeq ($(USE_EXPAND_RTWLIB),1)
	...
	$(LIBCMD) $(LIBCMD_ARGS) $(MODULES_rtwlib)
	@echo ### Created $@
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
# Rule for MODELREF_LINK_LIBS_MAIN
#------------------------------------------------------------------------------
ifeq "$(UNAME)" "Linux"	##### Linux (RedHawk)
...
MODELREF_LINK_LIBS_MAIN :
	@FOR %A IN ($(MODELREFS)) DO $(MAKE) -I "$(current_dir)" -C "$(SHARED_SRC_DIR)/../%A" -f %A.mk
endif
#------------------------------------------------------------------------------
