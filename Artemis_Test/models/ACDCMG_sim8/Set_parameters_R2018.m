...
% Autor: PhD(c). Leony Ortiz; Msc. Rogelio Orizondo; PhD (c). Alexander
% Aguila at al
% Modify: Fangyuan
...
%set_param('ACDCMG_sim8','IgnoredZcDiagnostic','none');  
%set_param('ACDCMG_2021b_sim7_work','IgnoredZcDiagnostic','none');  
 Vmin_mppt = 100;
 %set_param('DCMG_2','IgnoredZcDiagnostic','none'); 
 % Demand Function
...
% ****** SIMULINK - POWERGUI and Microgrid ******
    ...
    %Ts_Power=1/(24*Fnom)/1000*3; Ts = Ts_Power; %updated
    %% Increased Step Size to make the model run faster
    %% Real Ts_Power Line
    %Ts_Power=1/(24*Fnom)/100; Ts = Ts_Power;
    Ts = 40e-6; Ts_Power = Ts;
    %% end
   %Ts_Power=10/(24*Fnom)/100; Ts = Ts_Power; 
    % Ts_Power=1/4000/1000; Ts = Ts_Power; 
    % phase-phase rms
...
%    AC LOADs (Primary GRID)
        ...
        Ql_Load_12 = S_Load_12 * sin(acos(PF_Load_12)); 
          ...
          % add another primary load
        S_Load_13 = 80e3; % (kVA)
        PF_Load_13 = 0.6; % Power factor 
        P_Load_13 = S_Load_13* PF_Load_13; % (W)
        Ql_Load_13 = S_Load_13 * sin(acos(PF_Load_13)); 

          % add another primary load
        S_Load_15 = 200e3; % (kVA)
        PF_Load_15 = 0.6; % Power factor 
        P_Load_15 = S_Load_15* PF_Load_15; % (W)
        Ql_Load_15 = S_Load_15 * sin(acos(PF_Load_15)); 
        
        % modify load 11 as secondary load
        % S_Load_11 = 400e3; % (kVA)
        S_Load_11 = 12e3; % (kVA)
        PF_Load_11 = 0.6; % Power factor  original:0.8
...
%    line length (Km) (Primary GRID)
    ...
    dis8 = 2; 
    %dis9 = 6; 
    dis10 = 6; dis11 = 3; dis12 = 3; dis13 = 3; dis14 = 2; 
    
%    line length (Km) (Secondary GRID)
    % Rls = 0.198; Xls = 0.1089;
    Rls = 0.013; Xls = 0.93e-3;
    dis1 = 0.15; dis2 = 0.2; dis3 = 0.15; dis4 = 0.2; dis5 = 0.25; dis6 = 0.1; dis7 = 0.1;  
    dis9 = 0.15;
...
% CONTROL PARAMETERS
  ...
  P_nom_inv = 3e3; 
  H_invs = 6; % normalized inertia 
  D_invs = 0.05;   % normalized damping
