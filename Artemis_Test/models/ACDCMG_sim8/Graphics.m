...
% Reading of data
    ...
    m_bus = (length(VoltageLL_pu_microgrid)-1)/Phase; 
    %m_bus = length(VoltageLL_pu_microgrid)/Phase; 
    bus = 1:1:m_bus+1;
   % bus = 1:1:m_bus+1;
...
%  Interpolation
%     h=0.00001; Xi = 1:h:m_bus+1; 
%     y11 = interp1(bus,VoltageLL_pu_microgrid_plot (:,2),Xi,'spline'); z=0; c=0;
    
%    f22 = plot(Xi,y11,'b:',z,c,'linewidth',1.2);
    title('Voltage L-L (pu) per Bus  (abc)  ','fontname','times new roman','fontsize',13);
    xlabel('Bus number','fontname','times new roman','fontsize',13); 
    ylabel('Voltage (rms) abc L-L (pu)','fontname','times new roman','fontsize',13);  
...
% %     %m_bus = length(Q_abc_microgrid)/Phase; 
% %     %bus = 1:1:m_bus; 
% %     m_bus = (length(Q_abc_microgrid)+1)/Phase; 
% %     bus = 1:1:m_bus; 
% %     
% %     cont = 1;
% %         for j = 1 : m_bus 
...
% %     set(leyenda.BoxFace,'ColorType', 'truecoloralpha', 'ColorData',uint8([255;255;255;0.85*255]));
% %     axis([0.5 10.5 0.65 1.15]); % Change axis properties
% % 
...
% Graphic Power_Losse_abc "HISTOGRAM"
    figure(4); grid minor; hold on;
    f6 = bar(lines,Power_Losse_microgrid_plot);
    h=0.00001; Xi = 1:h:n_lines; 
...
% %    % switch Demand_case
% %      %   case 1 % Max demand
% %             axis([0.5 9.5 0 10]); % Max demand
% %    %     otherwise % Min demand
% %     %        axis([0.5 8.5 0 2]); % Min demand
% %    % end
% %     ax = gca;
...
% %     figure(5); hold on; grid minor;
% %     x = [0 m_bus+0.5];
% %     line(x,[1 1],'Color','green','LineStyle','-');
% %     PF_abc_microgrid_plot = abs(PF_abc_microgrid_plot);
% %     f7 = stem(bus,PF_abc_microgrid_plot,'o','linewidth',1.3);
% % %     h=0.00001; Xi = 1:h:m_bus; 
...
% %     f84 = stem(bus,THDv_abc_microgrid_plot,'o','linewidth',1.3);
